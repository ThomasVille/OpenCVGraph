...
public:
	...
	void LoadDll(const char* name, short showMsg = 1)
	{
		...
		wchar_t wtext[2000];
		mbstowcs(wtext, m_dllName, strlen(m_dllName) + 1);//Plus null
		...
		{
			std::cerr << "Could not load dll: " << m_dllHandle << std::endl;
		}
		else {

			std::cerr << "Sucsessfully loaded dll: "; wprintf(dllName); std::cout << " with the handle: " << m_dllHandle << std::endl;
		}
	}
